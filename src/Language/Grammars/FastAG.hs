...
comAspect ::
 ...
 =>  Aspect al -> Aspect ar -> Aspect asp
comAspect al ar
  = req emptyContext (OpComAsp al ar)
...
instance Require (OpComAsp al '[]) '[] where
  type ReqR (OpComAsp al '[]) = Aspect al
  req ctx (OpComAsp al _) = al

instance
  ( RequireR (OpComAsp al ar) '[] (Aspect ar')
  , Require  (OpComRA prd sc ip ic sp ic' sp' ar') '[]
  )
  => Require (OpComAsp al
       ( '(prd, Rule prd sc ip ic sp ic' sp') ': ar)) '[] where
  type ReqR (OpComAsp al
       ( '(prd, Rule prd sc ip ic sp ic' sp') ': ar))
    = ReqR (OpComRA prd sc ip ic sp ic' sp'
            (UnWrap (ReqR (OpComAsp al ar))))
  req ctx (OpComAsp al (ConsRec prdrule ar))
   = req emptyContext (OpComRA (untagField prdrule)
                      (req emptyContext (OpComAsp al ar)))


data OpComRA  (prd  :: Prod)
              (sc   :: [(Child, [(Att, Type)])])
              (ip   :: [(Att, Type)])
              (ic   :: [(Child, [(Att, Type)])])
              (sp   :: [(Att, Type)])
              ...
              (sp'  :: [(Att, Type)])
              (a     :: [(Prod, Type)])  where
  OpComRA :: Rule prd sc ip ic sp ic' sp'
          -> Aspect a -> OpComRA prd sc ip ic sp ic' sp' a
...
data OpComRA' (b :: Bool)
              ...
              (sp'  :: [(Att, Type)])
              (a     :: [(Prod, Type)])  where
  OpComRA' :: Proxy b -> Rule prd sc ip ic sp ic' sp'
          -> Aspect a -> OpComRA' b prd sc ip ic sp ic' sp' a


instance
 (Require (OpComRA' (HasLabel prd a) prd sc ip ic sp ic' sp' a) '[])
  => Require (OpComRA prd sc ip ic sp ic' sp' a) '[] where
  type ReqR (OpComRA prd sc ip ic sp ic' sp' a)
     = ReqR (OpComRA' (HasLabel prd a) prd sc ip ic sp ic' sp' a)
  req ctx (OpComRA rule a)
     = req ctx (OpComRA' (Proxy @ (HasLabel prd a)) rule a)

instance
  (Require (OpExtend PrdReco prd (Rule prd sc ip ic sp ic' sp') a)) '[]
  => Require (OpComRA' 'False prd sc ip ic sp ic' sp' a) '[] where
  type ReqR (OpComRA' 'False prd sc ip ic sp ic' sp' a)
    = ReqR (OpExtend PrdReco prd (Rule prd sc ip ic sp ic' sp') a)
  req emptyCtx (OpComRA' _ (rule :: Rule prd sc ip ic sp ic' sp') asp)
    = req emptyCtx (OpExtend (Label @ prd) rule asp)

instance
 ( Require (OpUpdate PrdReco prd (Rule prd sc ip ic sp ic'' sp'') a) '[]
 , RequireR (OpLookup PrdReco prd a) '[] (Rule prd sc ip ic sp ic' sp') 
 , (IC (ReqR (OpLookup PrdReco prd a))) ~ ic
 , (SP (ReqR (OpLookup PrdReco prd a))) ~ sp
 ) =>
  Require (OpComRA' 'True prd sc ip ic' sp' ic'' sp'' a) '[] where
  type ReqR (OpComRA' 'True prd sc ip ic' sp' ic'' sp'' a)
    = ReqR (OpUpdate PrdReco prd
           (Rule prd sc ip
             (IC (ReqR (OpLookup PrdReco prd a)))
             (SP (ReqR (OpLookup PrdReco prd a)))
            ic'' sp'') a)
  req emptyCtx (OpComRA' _ rule asp)
    = let prd     = Label @ prd
          oldRule = req emptyCtx (OpLookup prd asp)
          newRule = rule `ext` oldRule
      in  req emptyCtx (OpUpdate prd newRule asp)
